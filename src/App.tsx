...
export default function App() {
  ...
  const [view, setView] = useState<"mail" | "assignments" | "calendar">("mail");
...
function CalendarBoard({ assignments, events, query, onAdd, onEdit, onDelete }: { assignments: AssignmentDeadline[]; events: CalendarEvent[]; query: string; onAdd: (value: PersonalReminderInput) => void; onEdit: (value: PersonalReminderInput) => void; onDelete: (id: string) => Promise<void> }) {
  ...
  const merged = useMemo<CalendarEvent[]>(() => [
    ...
    ...events,
  ].filter((item) => `${item.title} ${item.notes}`.toLowerCase().includes(query.toLowerCase())), [assignments, events, query]);
  const grid = useMemo(() => buildMonthGrid(month), [month]);
  const dated = merged.filter((item) => item.startsAt);
  const unscheduled = merged.filter((item) => !item.startsAt);
  const selectedEvents = dated.filter((item) => localDateKey(item.startsAt!) === selectedDay).sort((a, b) => new Date(a.startsAt!).getTime() - new Date(b.startsAt!).getTime());
  const createForDay = (key: string) => {
    const [year, monthValue, day] = key.split("-").map(Number);
    const date = new Date(year, monthValue - 1, day, 18, 0, 0, 0);
  ...
  return <section className="calendar-board">
    ...
    <div className="calendar-layout"><div className="month-calendar"><div className="weekday-row">{["周一","周二","周三","周四","周五","周六","周日"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{grid.map((date) => { const key = localDateKey(date); const dayEvents = dated.filter((item) => localDateKey(item.startsAt!) === key); const currentMonth = date.getMonth() === month.getMonth(); const isToday = key === localDateKey(today); return <div key={key} className={`calendar-day ${currentMonth ? "" : "outside"} ${selectedDay === key ? "selected-day" : ""}`} onClick={() => setSelectedDay(key)}><div className="day-number"><span className={isToday ? "today" : ""}>{date.getDate()}</span><button aria-label={`在 ${key} 添加提醒`} onClick={(event) => { event.stopPropagation(); createForDay(key); }}><Plus size={12}/></button></div><div className="day-events">{dayEvents.slice(0, 3).map((item) => <button key={item.id} className={`calendar-chip ${item.kind} priority-${item.priority}`} title={item.title} onClick={(event) => { event.stopPropagation(); setSelectedDay(key); }}><time>{new Date(item.startsAt!).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time>{item.title}</button>)}{dayEvents.length > 3 && <span className="more-events">还有 {dayEvents.length - 3} 项</span>}</div></div>; })}</div></div>
      <aside className="day-agenda"><div className="agenda-heading"><div><span>{new Date(`${selectedDay}T12:00:00`).toLocaleDateString("zh-CN", { weekday: "long" })}</span><strong>{new Date(`${selectedDay}T12:00:00`).toLocaleDateString("zh-CN", { month: "long", day: "numeric" })}</strong></div><button onClick={() => createForDay(selectedDay)}><Plus size={14}/></button></div>{selectedEvents.length ? <div className="agenda-list">{selectedEvents.map((item) => <article className={`agenda-event ${item.kind}`} key={item.id}><div className="agenda-time">{new Date(item.startsAt!).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</div><div className="agenda-copy"><span>{eventKindLabel(item.kind)}</span><h3>{item.title}</h3>{item.notes && <p>{item.notes}</p>}<div className="agenda-tools">{item.kind === "personal" && <><button onClick={() => onEdit({ id: item.id, title: item.title, notes: item.notes, startsAt: item.startsAt!, priority: item.priority })}><Pencil size={12}/>编辑</button><button className="danger" onClick={() => { if (window.confirm("确定删除这个个人提醒吗？")) void onDelete(item.id); }}><Trash2 size={12}/>删除</button></>}{item.sourceUrl && <button onClick={() => navigator.clipboard.writeText(item.sourceUrl!)}><Copy size={12}/>复制链接</button>}</div></div></article>)}</div> : <div className="agenda-empty"><CalendarDays size={28}/><p>这一天还没有安排</p><button onClick={() => createForDay(selectedDay)}>添加个人提醒</button></div>}{unscheduled.length > 0 && <div className="unscheduled"><strong>未安排日期</strong>{unscheduled.map((item) => <span key={item.id}>{item.title}</span>)}</div>}</aside>
    </div>
  </section>;
}
