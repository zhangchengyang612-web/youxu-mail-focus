...
- 英文邮件详情提供按需“翻译为中文”；不自动翻译，只有点击后才将当前邮件正文发送到 MyMemory 翻译服务。
- 通过 BNBU 官网公开搜索接口按教授姓名查找邮箱，支持复制、直接写信和核对官方来源。
- MIME 正文提取、HTML 转纯文本、SQLite 持久化和系统钥匙串凭据存储。
- 待办、学业、校园事务、社团活动、实习、个人、外部七类关键词分类；支持发件人、域名、主题和正文自定义规则。
...
当前可分享的 Apple Silicon 测试包位于 `release/邮序_0.6.7_BNBU_macOS_AppleSilicon.dmg`。该版本已完成磁盘映像校验，但未使用 Apple Developer ID 公证；其他电脑首次运行时可能需要右键应用并选择“打开”。
...
数据库位于 Tauri 应用数据目录下的 `mail-focus.sqlite`。除用户主动点击英文邮件的“翻译为中文”外，邮件正文只在本机用于分类和提醒提取，不调用云端 AI。翻译时当前正文会发送到 MyMemory，译文只在当前应用会话内显示，不写入数据库。应用不加载 HTML 邮件中的远程图片，也不下载附件。可通过应用设置清除本地数据。
